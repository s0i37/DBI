...
void exec_instr(ADDRINT addr, ADDRINT nextAddr, CONTEXT * ctx)
{
...
bool read_from_pipe()
{
    if(afl_data_fd == -1)
   	...
   	fuzz_data.len = read(afl_data_fd, fuzz_data.data, FUZZ_DATA_SIZE);  /* WAIT */
    return true;
}
#endif
...
#elif __linux__
bool signal_recv(unsigned int threadId, int sig, CONTEXT* ctx, bool hasHandler, const EXCEPTION_INFO* pExceptInfo, void* v)
{
...
int main(int argc, char ** argv)
{
    ...
    #elif __linux__
    PIN_InterceptSignal(SIGSEGV, signal_recv, 0);
    #endif
